...
\begin{document}
	...
	Отсюда: \begin{align*}
		l(\bar{e}_{1}) = \frac{F(x_0, y_0 + h) - F(x_0, y_0) + \overline{\overline{o}}(h)}{h} = \lim\limits_{h \rightarrow 0} \frac{F(x_0, y_0 + h) - F(x_0, y_0)}{h} = \\ = \frac{\partial F}{\partial x}(x_0, y_0),
	\end{align*}
	...
	Будет в ближайшем обновлении, а сейчас я иду спать, потому что обрабатывать информацию пока не способен.\\
	Если вы дошли до этого места, спасибо, что цените мой труд)
	
	А ещё отдельным файлом публикую записи по проективке, за которые огромное спасибо Егору Цыбулину)
	Сам я их пока не отсмотрел, но правки мне можно присылать, пока я пишу это дело)
